...
let insert table proto left right translate timeout =
  let open Hashtbl in
  let internal_lookup = (proto, left, right) in
  let external_lookup = (proto, right, translate) in
  ...
  | false, false ->
    (* TODO: it's not clear to me that this is a great place for this logic *)
    (* timeout thread for these entries *)
    (* both entries point to the same thread, which seems OK because the thread
       immediately removes the only entries by which it could be accessed? *)
